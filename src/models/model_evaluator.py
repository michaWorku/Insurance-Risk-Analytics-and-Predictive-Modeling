...
def evaluate_classification_model(y_true: np.ndarray, y_pred_proba: np.ndarray, threshold: float = 0.5) -> Dict[str, float]:
    """
    Evaluates a binary classification model using accuracy, precision, recall, and F1-score.
    ...
    if len(y_true) == 0 or len(y_pred_proba) == 0:
        ...
        return {'Accuracy': np.nan, 'Precision': np.nan, 'Recall': np.nan, 'F1-score': np.nan}
